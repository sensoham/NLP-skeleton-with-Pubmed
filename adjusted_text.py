...
df_rem = pd.read_csv('Terms_det_prior.csv', header = 'infer', sep = ',')
...
df_sort = df_sort.drop_duplicates(subset=['OCCUR', 'PMID', 'Cum_freq_norm', 'date'], keep = 'first') # bit risky
df_sort = df_sort.drop_duplicates(subset=['TERM'], keep = 'first')


new_plot_df = df_sort[['TERM', 'date', 'Cum_freq_norm', 'value']].copy()
new_plot_df = new_plot_df.drop_duplicates()

new_plot_df.to_csv('C:/Users/soham.s/Desktop/Code/xmltagging/run_together/testings/TstDF.csv', sep =',')

new_plot_df = new_plot_df.sort_values(['value'], ascending = False)
